...
def process_personal_emotion_points_data(df):
    ...
    logger.info(f"Emotion ratios: {emotion_ratios}")
    return emotion_ratios

def plot_personal_emotion_radar_chart(personal_emotion_points_df, mem_num):
    logger.info(f"Plotting emotion radar chart for member {mem_num}")
    ...
    # 파일로 저장
    logger.info(f"Saving emotion radar chart to {file_path}")
    fig.savefig(file_path)

    plt.close()
    logger.info("Emotion radar chart plotting completed")